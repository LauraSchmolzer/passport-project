...
class ScoreCSCABuilder:
    ...
    def _score(self, csca_cert: CSCACertificate):
        ...
        if self.thumbprint_check(csca_cert):
            score += 1
        if self.has_links(csca_cert):
            score += 1
        if self.multiple_mls(csca_cert):
            score +=1
    ...
    def thumbprint_check(self,csca_cert: CSCACertificate) -> bool:
        return any(ml.hash_validated for ml in csca_cert.master_lists)
    ...
    def multiple_mls(self,csca: CSCACertificate) -> bool:
